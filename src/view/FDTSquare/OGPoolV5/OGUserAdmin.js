                ...
                refArr.forEach(refItem => {
                    for (let j = 0; j < recordArr.length; j++) {
                        ...
                        if (recordItem.addr.toLowerCase() == refItem._user.toLowerCase()) {
                            ...
                            recordItem.rate = getLevelRate(refItem.level)
                            teamRecordArr.push(recordItem)
                            totalETH = BigNumber(totalETH).plus(recordItem.ethAmount / 10 ** ETHDecimals)
                            totalFLM = BigNumber(totalFLM).plus(recordItem.flmAmount / 10 ** FLMDecimals)
                        }
                    }
                })
